...
export function RekomendasiPtPage() {
  ...
  const handleGenerate = async () => {
    ...
    setResult('')
    try {
      const res = await fetch('/api/rekomendasi-pt', {
        method: 'POST',
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base">Rekomendasi Perguruan Tinggi</CardTitle>
          </div>
          <CardDescription>
            Pilih siswa untuk mendapatkan rekomendasi perguruan tinggi berdasarkan nilai, jurusan minat, dan profil akademik
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-end gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-[200px] space-y-2">
              <Label>Jurusan Minat (Opsional)</Label>
              <Input
                placeholder="Contoh: Teknik Informatika"
                value={jurusanMinat}
                onChange={e => setJurusanMinat(e.target.value)}
              />
            </div>
            <Button onClick={handleGenerate} disabled={!selectedSiswa || generating}>
              {generating ? (
                <>
                  <Loader2 className="h-4 w-4 mr-1 animate-spin" /> Mencari...
                </>
              ) : (
                <>
